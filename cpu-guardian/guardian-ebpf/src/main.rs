...
fn try_guardian_exec(ctx: TracePointContext) -> Result<u32, i64> {
    ...
    // Capture the filename being executed (at offset 16 for sys_enter_execve)
    let filename_ptr: *const u8 = unsafe { ctx.read_at(16)? };
    unsafe {
        bpf_probe_read_user_str_bytes(filename_ptr, &mut event.data.exec.comm)?;
...
fn try_guardian_connect(ctx: TracePointContext) -> Result<u32, i64> {
    ...
    // addr is at offset 24 for sys_enter_connect
...
fn try_guardian_openat(ctx: TracePointContext) -> Result<u32, i64> {
    ...
    // filename is at offset 24 for sys_enter_openat
