...
use guardian_common::{
    ConnectEvent, EventData, ExecEvent, ForkEvent, GuardianEvent, OpenEvent, EVENT_TYPE_CONNECT,
    EVENT_TYPE_EXEC, EVENT_TYPE_FORK, EVENT_TYPE_OPEN,
};
...
fn try_guardian_connect(ctx: TracePointContext) -> Result<u32, i64> {
    ...
    if sa.sa_family == 2 {
        // AF_INET
        let sin: sockaddr_in = unsafe { core::mem::transmute(sa) };
        let event = GuardianEvent {
