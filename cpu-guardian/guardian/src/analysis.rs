...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // 10 base + 10 for .pdf = 20 points
        analyzer.handle_event(event);
        assert_eq!(analyzer.processes.get(&1234).unwrap().score, 20);

        // Another sensitive delete
        let filename2 = b"notes.txt\0";
        unsafe {
            event.data.unlink.filename = [0; 64]; // Reset
            event.data.unlink.filename[..filename2.len()].copy_from_slice(filename2);
        }
        analyzer.handle_event(event);
        assert_eq!(analyzer.processes.get(&1234).unwrap().score, 40);
    }
}
