        ...
        match event.event_type {
            ...
            EVENT_TYPE_UNLINK | EVENT_TYPE_UNLINKAT => {
                ...
                let filename = if event.event_type == EVENT_TYPE_UNLINK {
                    ...
                    std::str::from_utf8(&unlink.filename)
                        .unwrap_or("")
                        .trim_matches(char::from(0))
                } else {
                    let unlinkat = unsafe { &event.data.unlinkat };
                    std::str::from_utf8(&unlinkat.filename)
                        .unwrap_or("")
                        .trim_matches(char::from(0))
                };
...
mod tests {
    use super::*;
    use guardian_common::{ExecEvent, ForkEvent, UnlinkEvent, EventData, EVENT_TYPE_EXEC, EVENT_TYPE_FORK, EVENT_TYPE_UNLINK};
    ...
    fn test_unlink_scoring() {
        ...
        // Deleting a regular file should give 10 points
        let filename = b"regular_file.png\0";
        unsafe {
            event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }
        assert_eq!(analyzer.handle_event(event), None);

        // Deleting a document should give 20 points total (10 base + 10 document)
        let doc_filename = b"secret.pdf\0";
        unsafe {
            event.data.unlink.filename = [0; 64]; // Reset filename
