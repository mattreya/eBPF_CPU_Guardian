...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        let mut unlink_event = GuardianEvent {
        ...
        // Deleting a normal file: 10 points
        analyzer.handle_event(unlink_event);
        assert_eq!(analyzer.processes.get(&pid).unwrap().score, 10);

        // Deleting a document: +10 more points (total 20 more for this event, 30 cumulative)
        let filename = b"secret.pdf\0";
        unsafe {
            unlink_event.data.unlink.filename = [0; 64];
            unlink_event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }

        assert_eq!(analyzer.handle_event(unlink_event), Some(pid));
