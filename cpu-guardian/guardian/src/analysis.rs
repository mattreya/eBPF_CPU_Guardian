...
use guardian_common::{
    GuardianEvent, EVENT_TYPE_EXEC, EVENT_TYPE_CONNECT, EVENT_TYPE_OPEN,
    EVENT_TYPE_FORK, EVENT_TYPE_UNLINK, EVENT_TYPE_UNLINKAT,
    ...
    pub fn handle_event(&mut self, event: GuardianEvent) -> Option<u32> {
        match event.event_type {
            ...
            EVENT_TYPE_OPEN => {
                ...
                let filename = unsafe { &event.data.open.filename };
                let filename_str = std::str::from_utf8(filename)
                    .unwrap_or("")
                    .trim_matches(char::from(0));
                if check_sensitive_extension(filename_str) {
                    state.score += 10;
                }
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // 10 base + 10 for .pdf = 20 (threshold)
        assert_eq!(analyzer.handle_event(event), Some(1234));
    }
}
