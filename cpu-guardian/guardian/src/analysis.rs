...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Deleting a document (10 + 10 = 20 points)
        let mut doc_event = event;
        let doc_filename = b"secret.pdf\0";
        unsafe {
            doc_event.data.unlink.filename = [0; 64];
            doc_event.data.unlink.filename[..doc_filename.len()].copy_from_slice(doc_filename);
        }
        assert_eq!(analyzer.handle_event(doc_event), Some(5678));
    }
}
