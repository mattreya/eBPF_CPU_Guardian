...
use guardian_common::{
    ...
    EVENT_TYPE_UNLINK, EVENT_TYPE_UNLINKAT, EventData, UnlinkEvent
};
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Unlink non-document file (10 points)
        let filename = b"temp_file.tmp\0";
        unsafe {
            event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }
        assert_eq!(analyzer.handle_event(event), None);

        // Unlink document file (10 base + 10 doc = 20 points, total 30)
        ...
        assert_eq!(analyzer.handle_event(event), Some(2000));
    }
}
