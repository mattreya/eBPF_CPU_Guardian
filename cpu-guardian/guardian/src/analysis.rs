...
impl Analyzer {
    ...
    pub fn handle_event(&mut self, event: GuardianEvent) -> Option<u32> {
        if event.event_type == EVENT_TYPE_FORK {
            ...
            let _child_state = self.processes.entry(fork.child_pid).or_insert(ProcessState {
                pid: fork.child_pid,
        ...
        match event.event_type {
            EVENT_TYPE_EXEC => {
                ...
                if comm_lower.contains("chromium") ||
                   comm_lower.contains("firefox") ||
                   comm_lower.contains("headless") ||
                   comm_lower.contains("openclaw") ||
                   comm_lower.contains("clawbot") ||
                   comm_lower.contains("moltbot") {
                    state.score += 50;
                }
            }
            EVENT_TYPE_CONNECT => {
...
mod tests {
    ...
    fn test_inheritance() {
        ...
        // Child should be automatically flagged
