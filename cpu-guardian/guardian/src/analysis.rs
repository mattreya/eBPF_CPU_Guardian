...
use guardian_common::{
    ...
    EVENT_TYPE_UNLINK, EVENT_TYPE_UNLINKAT, ExecEvent, ForkEvent, UnlinkEvent, EventData
};
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Mock a normal file deletion
        let filename = b"test.log\0";
        unsafe {
            event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }
        assert_eq!(analyzer.handle_event(event), None); // Score should be 10

        // Mock a document deletion
        let mut event2 = event;
        let doc_filename = b"secret.pdf\0";
        unsafe {
            event2.data.unlink.filename = [0; 64];
            event2.data.unlink.filename[..doc_filename.len()].copy_from_slice(doc_filename);
        }
        assert_eq!(analyzer.handle_event(event2), Some(1234)); // Score should be 10 + 20 = 30, which is > 20
    }
}
