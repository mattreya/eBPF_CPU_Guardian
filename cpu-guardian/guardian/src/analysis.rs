...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Deleting a non-document file should give 10 points
        let filename = b"test.bin\0";
        unsafe {
            event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }
        assert_eq!(analyzer.handle_event(event), None);

        // Deleting a document file should give 20 points total (10 base + 10 document)
        let filename = b"secret.pdf\0";
        unsafe {
            // Need to clear the buffer first because of previous test or just overwrite
