...
use guardian_common::{
    ...
    EventData, UnlinkEvent, ExecEvent, ForkEvent,
};
    ...
    pub fn handle_event(&mut self, event: GuardianEvent) -> Option<u32> {
        match event.event_type {
            ...
            EVENT_TYPE_UNLINK | EVENT_TYPE_UNLINKAT => {
                ...
                let filename = std::str::from_utf8(filename_raw)
                    .unwrap_or("")
                    .trim_matches(char::from(0));
                if check_sensitive_extension(filename) {
                    state.score += 10;
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        let filename_doc = b"secret.pdf\0";
        unsafe {
            event_doc.data.unlink.filename[..filename_doc.len()].copy_from_slice(filename_doc);
        }
        analyzer.handle_event(event_doc);
        // 10 (base) + 10 (doc) = 20 more. Total 30.
