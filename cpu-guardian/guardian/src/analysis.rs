...
use guardian_common::{
    ...
    EVENT_TYPE_UNLINK, EVENT_TYPE_UNLINKAT, EventData, ExecEvent, ForkEvent, UnlinkEvent
};
        ...
        match event.event_type {
            ...
            EVENT_TYPE_OPEN => {
                ...
                if filename.to_lowercase().ends_with(".pdf")
                    ...
                    || filename.to_lowercase().ends_with(".doc")
                {
                    state.score += 10;
                }
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Deleting a normal file should give 10 points (not enough for threshold 20)
        let filename = b"random.dat\0";
        unsafe { event.data.unlink.filename[..filename.len()].copy_from_slice(filename); }
        assert_eq!(analyzer.handle_event(event), None);

        // Deleting a document should give 10+10 = 20 points, triggering detection
        let doc_filename = b"secret.pdf\0";
        unsafe {
            // Need to reset filename buffer to avoid leftover characters
            event.data.unlink.filename = [0; 64];
            event.data.unlink.filename[..doc_filename.len()].copy_from_slice(doc_filename);
        }
        assert_eq!(analyzer.handle_event(event), Some(555));
    }
}
