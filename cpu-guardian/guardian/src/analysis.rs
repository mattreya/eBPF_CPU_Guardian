            ...
            EVENT_TYPE_UNLINK | EVENT_TYPE_UNLINKAT => {
                ...
                let filename = unsafe {
                    if event.event_type == EVENT_TYPE_UNLINK {
                        std::str::from_utf8(&event.data.unlink.filename)
                    } else {
                        std::str::from_utf8(&event.data.unlinkat.filename)
                    }
                }.unwrap_or("").trim_matches(char::from(0));
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        // Deleting a non-document file (10 points)
        let filename = b"malware.sh\0";
        unsafe {
            event.data.unlink.filename[..filename.len()].copy_from_slice(filename);
        }
        assert_eq!(analyzer.handle_event(event), None);

        // Deleting a document file (another 10 + 10 = 20 total)
        let doc_filename = b"SENSITIVE.PDF\0";
        unsafe {
            event.data.unlink.filename = [0; 64]; // Clear previous
