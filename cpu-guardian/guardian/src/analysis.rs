...
use guardian_common::{GuardianEvent, EventData, UnlinkEvent, EVENT_TYPE_EXEC, EVENT_TYPE_CONNECT, EVENT_TYPE_OPEN, EVENT_TYPE_FORK, EVENT_TYPE_UNLINK, EVENT_TYPE_UNLINKAT};
...
mod tests {
    ...
    fn test_unlink_scoring() {
        ...
        let filename = b"test.tmp\0";
        unsafe { event.data.unlink.filename[..filename.len()].copy_from_slice(filename); }

        analyzer.handle_event(event);
        assert_eq!(analyzer.processes.get(&pid).unwrap().score, 10);

        // Test document file unlink
        let mut doc_event = GuardianEvent {
        ...
        unsafe { doc_event.data.unlink.filename[..doc_filename.len()].copy_from_slice(doc_filename); }

        analyzer.handle_event(doc_event);
        // Previous 10 + (10 base + 10 doc bonus) = 30
        assert_eq!(analyzer.processes.get(&pid).unwrap().score, 30);
    }
}
